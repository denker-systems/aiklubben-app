...
```typescript
const { data, error } = await supabase.storage.from('avatars').remove([`${userId}/avatar.png`]);
```
...
const { data } = await supabase.from('profiles').select('*'); // Bypasses RLS with service role key
```
...
```typescript
const { data } = await supabase.from('news').select('*').limit(20);
```

### Pagination

```typescript
const { data } = await supabase.from('news').select('*').range(0, 19); // First 20 items
```
